...
namespace API.Application.Reservations
{
    ...
    public class ConfirmReservationCommandHandler : IRequestHandler<ConfirmReservationCommand, Unit>
    {
        ...
        public ConfirmReservationCommandHandler(IDocumentSession documentSession, ISendEmail sendGridEmailSender, IConfiguration configuration)
        {
        ...
        public async Task<Unit> Handle(ConfirmReservationCommand request, CancellationToken cancellationToken)
        {
            ...
            if (reservation != null && restaurant != null)
            {
                ...
                await SendEmailConfirmationToClientAsync(reservation, restaurant);
            }

            return Unit.Value;
        }

        private async Task SendEmailConfirmationToClientAsync(Reservation reservation, Restaurant restaurant)
        {
            var data = new
            {
            ...
            await _sendGridEmailSender.SendAsync(reservation.ReservatorEmail, _configuration["SendGrid:ConfirmationReservationTemplateId"], data);
