...
@Component({
  selector: 'app-list-produit-supermarche',
  templateUrl: './list-produit-supermarche.component.html',
  styleUrls: ['./list-produit-supermarche.component.css']
})

export class ListProduitSupermarcheComponent implements OnInit {
  ...
  getProduits() {
    ...
    this.serviceSupermarche.getSupermarcheById(this.idSupermarche).subscribe((data) => {
      this.supermarche = data;

      this.produitService.getProduitsByIdSupermarket(data.id).subscribe((response) => {
        this.prod = response
  ...
  suppressionProduitByIdProduitAndIdCard(produit: Produit) {
    this.produitService.deleteProduit(produit.supermarketId, produit.id).subscribe((response) => {
