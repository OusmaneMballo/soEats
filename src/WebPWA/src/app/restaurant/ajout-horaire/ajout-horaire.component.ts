...
export class AjoutHoraireComponent implements OnInit {
  ...
  //Fonction qui permet d'ajout des horaire d'un restaurant
  addHoraire() {
    let uuid_restaurant=this.data.id;
    if(this.horaireForm.valid===true){
      console.log(this.horaireForm.controls["slot2StartTime"].value)
          ...
          if((this.slot2.startTime!==null && this.slot2.endTime===null) || (this.slot2.startTime===null && this.slot2.endTime!==null)){
            this.openMessageDialog("creneau2");
              ...
              && this.horaireService.testCaractereHoraire(this.slot2.startTime)===true && this.horaireService.testCaractereHoraire(this.slot2.endTime)===true){
                ...
                //Appel d'API
                this.horaireService.addHoraire(this.horaire, uuid_restaurant).subscribe(
                  ()=>{
  ...
  horaireDuJour($event: any){
    ...
    this.horaireService.getOpenHoursBySlugId(uuid_restaurant).subscribe(data => {
      let trouv=false;
